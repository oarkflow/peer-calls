...
func (a *RedisAdapter) remove(clientID identifiers.ClientID) (err error) {
	...
	if err = a.pubRedis.HDel(context.Background(), a.keys.roomClients, clientID.String()).Err(); err != nil {
...
func (a *RedisAdapter) Metadata(clientID identifiers.ClientID) (metadata string, ok bool) {
	metadata, err := a.pubRedis.HGet(context.Background(), a.keys.roomClients, clientID.String()).Result()
...
func (a *RedisAdapter) SetMetadata(clientID identifiers.ClientID, metadata string) (ok bool) {
	...
	_, err := a.pubRedis.HSet(context.Background(), a.keys.roomClients, clientID.String(), metadata).Result()
	if err != nil {
		// FIXME return error
...
func (a *RedisAdapter) Clients() (map[identifiers.ClientID]string, error) {
	...
	r := a.pubRedis.HGetAll(context.Background(), a.keys.roomClients)
...
func (a *RedisAdapter) handleMessage(
	...
	handleRoomJoin := func(roomID identifiers.RoomID, join message.RoomJoin) error {
		...
		err = a.pubRedis.HSet(context.Background(), a.keys.roomClients, join.ClientID.String(), join.Metadata).Err()
		if err != nil {
	...
	handleRoomLeave := func(roomID identifiers.RoomID, clientID identifiers.ClientID) error {
		...
		err = a.pubRedis.HDel(context.Background(), a.keys.roomClients, clientID.String()).Err()
...
func (a *RedisAdapter) subscribe(ctx context.Context, ready chan<- struct{}) error {
	...
	pubsub := a.subRedis.PSubscribe(context.Background(), a.keys.roomChannel, a.keys.clientPattern)

	defer pubsub.Close()

	ch := pubsub.ChannelWithSubscriptions(redis.WithChannelSize(defaultSubscriptionChannelSize))
...
func (a *RedisAdapter) publish(channel string, msg message.Message) error {
	...
	err = a.pubRedis.Publish(context.Background(), channel, string(data)).Err()
